...
public class OpenInv extends JavaPlugin implements IOpenInv {
    ...
    public void reloadConfig() {
        ...
        super.reloadConfig();
        this.offlineHandler = disableOfflineAccess() ? OfflineHandler.REMOVE_AND_CLOSE
                : OfflineHandler.REQUIRE_PERMISSIONS;
    ...
    public void onDisable() {
        ...
        Stream.concat(inventories.values().stream(), enderChests.values().stream()).map(inventory -> {
    ...
    private void sendVersionError(@NotNull Consumer<String> messageMethod) {
        ...
        if (!this.accessor.isSupported()) {
            ...
            messageMethod
                    .accept("Please download the correct version of OpenInv here: " + this.accessor.getReleasesLink());
    ...
    public @Nullable OfflinePlayer matchPlayer(@NotNull String name) {
        ...
        // Warn if called on the main thread - if we resort to searching offline
        // players, this may take several seconds.
        ...
        // Exact offline match second - ensure offline access works when matchable users
        // are online.
    ...
    /**
     * Evict all viewers lacking cross-world permissions when a {@link Player}
     * changes worlds.
     *
    ...
    private void kickCrossWorldViewers(@NotNull Player player, @NotNull ISpecialInventory inventory) {
        ...
        ejectViewers(inventory, viewer -> !Permissions.CROSSWORLD.hasPermission(viewer)
     ...
     *
     * <p>
     * Note that this method is specifically for converting an
     * ISpecialPlayerInventory slot number into a regular player inventory slot
     * number.
     *
    ...
    private void setPlayerOffline(@NotNull Map<UUID, ? extends ISpecialInventory> map, @NotNull UUID key,
    ...
    void handleCloseInventory(@NotNull ISpecialInventory inventory) {
        ...
        Map<UUID, ? extends ISpecialInventory> map = inventory instanceof ISpecialPlayerInventory ? inventories
                : enderChests;
        ...
        if (loaded == null) {
            ...
            // Loaded inventory has already been removed. Removal will handle saving if
            // necessary.
            return;

        }

        // This should only be possible if a plugin is doing funky things with our
        // inventories.
        if (loaded != inventory) {
            ...
            // We need to close this inventory to reduce risk of duplication bugs if the
            // user is offline.
            // We don't want to risk recursively closing the same inventory repeatedly, so
            // we schedule dumping viewers.
            // Worst case we schedule a couple redundant tasks if several people had the
            // inventory open.
            if (inventory.isInUse()) {
        ...
        // Schedule task to check in use status later this tick. Closing user is still
        // in viewer list.
        getServer().getScheduler().runTask(this, () -> {
            ...
            if (disableSaving() || current == null || !(current.getPlayer() instanceof Player player)
                    || player.isOnline())
            {
    ...
    void setPlayerOnline(@NotNull Player player) {
        ...
        // New player may have a name that already points to someone else in lookup
        // cache.
        ...
        }, 101L); // Odd delay for pseudo load balancing; Player tasks are usually scheduled with
                  // full seconds.

    }

    private void setPlayerOnline(@NotNull Map<UUID, ? extends ISpecialInventory> map, @NotNull Player player,
            @Nullable Runnable task)
    {
        ...
        ejectViewers(inventory,
                viewer -> !Permissions.OPENONLINE.hasPermission(viewer) || !Permissions.CROSSWORLD.hasPermission(viewer)
